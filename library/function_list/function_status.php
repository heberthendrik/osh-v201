...
function GetStatusByID($input_parameter){
	...
	$query_get = "select * from master_status where id = '".$input_parameter['ID']."' ";
...
function GetAllStatus(){
	...
	if( $_SESSION['OSH']['ID_ROLE'] == 1 ){
		$query_get = "select * from master_status";
	} else {
		$query_get = "select * from master_status where ID_RS = '".$_SESSION['OSH']['ID_RS']."' ";
	}
	$result_get = $db->query($query_get);
	$num_get = $result_get->num_rows;
