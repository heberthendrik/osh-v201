...
function AddPetugas($input_parameter){
	...
	";
	$result_add_officer = $db->query($query_add_officer);
	if( $result_add_officer ){
		$query_status_officer = 1;
	} else {
		$query_status_officer = 0;
	}
	$new_officer_id = $db->insert_id;
	...
	if( $query_status_user == 1 && $query_status_officer == 1 && $query_status_composite == 1 ){
...
function UpdatePetugasByID($input_parameter){
	...
	";
	$result_getofficerid = $db->query($query_getofficerid);
	$row_getofficerid = $result_getofficerid->fetch_assoc();
	$id_officer = $row_getofficerid['ID_OFFICER'];
	$id_user = $row_getofficerid['ID_USER'];
...
/* 	echo '<br>'.$query_update_user; */
	...
	where
		ID = '".$id_officer."'
	";
/* 	echo '<br>'.$query_update_officer;exit; */
	$result_update_officer = $db->query($query_update_officer);
	if( $result_update_officer ){
		$query_status_update_officer = 1;
	} else {
		$query_status_update_officer = 0;
	}

	if( $query_status_update_user == 1 && $query_status_update_officer == 1 ){
		$function_result['FUNCTION_RESULT'] = 1;
		$function_result['SYSTEM_MESSAGE'] = "Data petugas telah berhasil diperbaharui." ;	
	} else {
		$function_result['FUNCTION_RESULT'] = 0;
...
function DeletePetugasByID($input_parameter){
	...
	$function_result['FUNCTION_RESULT'] = 1;
	$function_result['SYSTEM_MESSAGE'] = "Data petugas telah berhasil dihapus.";
...
function GetPetugasByID($input_parameter){
	...
	from master_user_composite t1
		...
		left join master_officer t3 on t3.ID = t1.ID_OFFICER
	where 
...
function EmptyPetugas(){
	...
	$function_result['MESSAGE'] = "Semua petugas point telah berhasil dihapus.";
