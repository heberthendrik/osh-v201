...
function GetRuangByID($input_parameter){
	...
	$query_get = "select * from master_room where id = '".$input_parameter['ID']."' ";
...
function GetAllRuang(){
	...
	if( $_SESSION['OSH']['ID_ROLE'] == 1 ){
		$query_get = "select * from master_room";
	} else {
		$query_get = "select * from master_room where ID_RS = '".$_SESSION['OSH']['ID_RS']."' ";
	}
	$result_get = $db->query($query_get);
	$num_get = $result_get->num_rows;
