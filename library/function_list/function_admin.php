...
function AddAdmin($input_parameter){
	global $db;
	...
	";
	$result_add_admin = $db->query($query_add_admin);
	if( $result_add_admin ){
		$query_status_admin = 1;
	} else {
		$query_status_admin = 0;
	}
	...
	if( $query_status_user == 1 && $query_status_admin == 1 && $query_status_composite == 1 ){
		$function_result['FUNCTION_RESULT'] = 1;
		$function_result['SYSTEM_MESSAGE'] = "Admin telah berhasil ditambahkan." ;
...
function UpdateAdminByID($input_parameter){
	global $db;
	...
	";
	$result_getadminid = $db->query($query_getadminid);
	$row_getadminid = $result_getadminid->fetch_assoc();
	$id_admin = $row_getadminid['ID_ADMIN'];
	$id_user = $row_getadminid['ID_USER'];
...
/* 	echo '<br>'.$query_update_user; */
	...
	where
		ID = '".$id_admin."'
	";
/* 	echo '<br>'.$query_update_admin;exit; */
	$result_update_admin = $db->query($query_update_admin);
	if( $result_update_admin ){
		$query_status_update_admin = 1;
	} else {
		$query_status_update_admin = 0;
	}

	if( $query_status_update_user == 1 && $query_status_update_admin == 1 ){
		$function_result['FUNCTION_RESULT'] = 1;
		$function_result['SYSTEM_MESSAGE'] = "Data admin telah berhasil diperbaharui." ;	
	} else {
		$function_result['FUNCTION_RESULT'] = 0;
...
function DeleteAdminByID($input_parameter){
	global $db;
	...
	$function_result['FUNCTION_RESULT'] = 1;
	$function_result['SYSTEM_MESSAGE'] = "Data admin telah berhasil dihapus.";
	
	return $function_result;
}

function GetAdminByID($input_parameter){
	global $db;
...
function EmptyAdmin(){
	...
	$function_result['MESSAGE'] = "Semua admin point telah berhasil dihapus.";
